...
export const InputComment = ({ mode, query, postId }: { mode: boolean; query: { refetch: Function }; postId: string }) => {
   ...
   return (
      <div className={`flex flex-row items-stretch w-full justify-between`}>
         <textarea
            className={`font-open text-sm w-[89%] outline-none ${
               mode
                  ? 'bg-[hsl(0,0%,20%)] text-[hsl(0,0%,90%)] p-1 placeholder:text-[hsl(0,0%,80%)]'
